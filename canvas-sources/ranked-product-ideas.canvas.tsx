...
function RankingDetail({
  ...
  return (
    <Card size="lg">
      <CardHeader>Разбор идеи</CardHeader>
      <CardBody>
        <Stack gap={16}>
          <Row gap={8} justify="space-between" align="center" wrap>
            <Row gap={7} wrap>
              <Pill active>№ {x.rank}</Pill>
              <Pill>{family(x)}</Pill>
            </Row>
          ...
          <Stack gap={6}>
            <H2>{x.titleRu}</H2>
            <Text tone="secondary" italic>{x.originalTerm}</Text>
            <Text>{x.oneLiner}</Text>
          </Stack>
          <Grid columns="repeat(auto-fit, minmax(130px, 1fr))" gap={10}>
            <Stat value={x.weightedScore.toFixed(1)} label="score"/>
            <Stat value={budget(x)} label="бюджет"/>
            <Stat value={`${x.estimatedMvpMonths} мес.`} label="MVP"/>
            <Stat value={confidence(x.confidence)} label="уверенность"/>
          </Grid>
          <Grid columns="repeat(auto-fit, minmax(220px, 1fr))" gap={16}>
            <List title="Драйверы score" items={x.rankingFactors.drivers}/>
            <List title="Штрафы score" items={x.rankingFactors.penalties}/>
          </Grid>
          <Callout
            tone={x.critique.verdict.includes("топ") ? "success" : "warning"}
            title={`Вердикт: ${x.critique.verdict}`}
          >
            {x.critique.verdictReason}
          </Callout>
          <Row gap={8} wrap>
            <Button onClick={() => onMode("Полный паспорт")}>Полный паспорт</Button>
            <Button variant="secondary" onClick={() => onMode("Критика и вопросы")}>
              Критика и вопросы
            </Button>
          </Row>
        </Stack>
...
function Ranking({
  ...
  const [q, setQ] = useCanvasState("search", "");
  const [fam, setFam] = useCanvasState("family", "all");
  const [seg, setSeg] = useCanvasState("segment", "all");
  const [ver, setVer] = useCanvasState("verdict", "all");
  const [conf, setConf] = useCanvasState("confidence", "all");
  const [min, setMin] = useCanvasState("threshold", "0");
  const [panelOpen, setPanelOpen] = useCanvasState(
    "ranking-detail-panel-open-v1",
    false,
  );

  const segments = Array.from(
    new Set(ideas.map(x => x.segment.split("/")[0].trim())),
  ).sort() as string[];
  const verdicts = Array.from(
    new Set(ideas.map(x => x.critique.verdict)),
  ).sort() as string[];
  const query = q.trim().toLocaleLowerCase("ru");
  const rows = ideas.filter(x =>
    (!query ||
      `${x.titleRu} ${x.originalTerm} ${x.segment}`
        .toLocaleLowerCase("ru")
        .includes(query)) &&
    (fam === "all" || family(x) === fam) &&
    (seg === "all" || x.segment.startsWith(seg)) &&
    (ver === "all" || x.critique.verdict === ver) &&
    (conf === "all" || x.confidence === conf) &&
    x.weightedScore >= Number(min)
  );
  const selected = rows.find(x => x.id === id) || rows[0];
  const showPanel = panelOpen && Boolean(selected);
  ...
  return (
    <Stack gap={18}>
      <H2>Рейтинг 50</H2>
      <Grid columns="repeat(auto-fit, minmax(175px, 1fr))" gap={10}>
        <TextInput
          type="search"
          value={q}
          onChange={setQ}
          placeholder="Поиск RU / original term"
        />
        <Select
          value={fam}
          onChange={setFam}
          options={[
            {value: "all", label: "B2B / B2C / B2B2C"},
            ...["B2B", "B2C", "B2B2C"].map(value => ({value, label: value})),
          ]}
        />
        <Select
          value={seg}
        ...
        <Select
          ...
          options={[
            ...
            ...verdicts.map(value => ({value, label: value})),
          ]}
        />
        <Select
          value={conf}
          onChange={setConf}
          options={[
            {value: "all", label: "Любая уверенность"},
        ...
        <Select
          value={min}
          onChange={setMin}
          options={["0", "70", "75", "80"].map(value => ({
            value,
            label: value === "0" ? "Любой score" : `Score ≥ ${value}`,
          }))}
        />
      </Grid>
      <Text tone="secondary">Показано {rows.length} из 50</Text>
      ...
      {!showPanel && (
        <Table
          ...
          rows={rows.map(x => [
            x.rank,
            <Stack gap={2}>
              <Text weight="semibold">{x.titleRu}</Text>
              <Text size="small" tone="tertiary">{x.originalTerm}</Text>
            </Stack>,
            x.segment,
            x.weightedScore.toFixed(1),
            budget(x),
            `${x.estimatedMvpMonths} мес.`,
            x.critique.verdict,
            confidence(x.confidence),
            <Button variant="secondary" onClick={() => openDetail(x.id)}>
              Разбор
            </Button>,
          ])}
        />
      ...
      {showPanel && selected && (
        <Grid
          columns="repeat(auto-fit, minmax(min(100%, 420px), 1fr))"
          gap={18}
          align="start"
        >
          <Stack gap={8} style={{maxHeight: 720, overflowY: "auto"}}>
            <Table
              striped
              stickyHeader
              headers={["№", "Продукт", ""]}
              rows={rows.map(x => [
                x.rank,
                <Stack gap={2}>
                  <Text weight={x.id === selected.id ? "bold" : "semibold"}>
                    {x.titleRu}
                  </Text>
                  <Text size="small" tone="tertiary">
                    Score {x.weightedScore.toFixed(1)}
                  </Text>
                </Stack>,
                <Button
                ...
                >
                  {x.id === selected.id ? "Открыто" : "Разбор"}
                </Button>,
              ])}
            />
          </Stack>
